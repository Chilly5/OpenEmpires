# Commander Phase 4C.2 Implementation Plan
...
## Task 2: Review, regression and phase gate

**Agents:** Luna evidence/regression; Sol independent task reviewer; Astra integration/security review.

- [ ] Luna creates actual scoped diff/new-source package against Task1 before snapshots and records source hashes; no production edits.
- [ ] Independent reviewer reads brief/report/diff and verifies spec plus code quality; no repeat suite for unchanged source. Fixes return to original implementer with scoped re-review, max five rounds per task.
- [ ] Luna runs full EditMode and full PlayMode on frozen final source once, including all Phase3/4A/4B/4C tests, preserving detailed results. A stale MCP counter requires same-handle/log/XML recovery, not automatic restart.
- [ ] Luna runs boundary/key audit and verifies frozen commands/network/executor/simulation unchanged. Host projection references are allowed; explanation service/value references are not. No key values printed.
- [ ] Astra verifies named tests and actual runtime/provenance/neutrality evidence, clean full suites and all substantive review findings. Record gate in progress ledger, then admit 4C.3 implementation.

## Preflight and risks

| Pair or task | Producer / consumer check | Resolution |
| --- | --- | --- |
| 4C.1 memory -> explanation host | RecordExplanation plus established reset lifecycle | No memory or bridge API changes required |
| Decision event -> values -> service | Mutable graph projected once into primitives | No source graphs in service or retained explanation source |
| Synchronous submission -> event attribution | Rejected decision may have null selected intent | Host-scoped copied provenance; clear in finally |
| Historical outcome -> current progress | Decision tick differs from fresh capture tick | Separate fields and explicit labels |
| Query routing -> pending preview | Existing submit path clears pending | Match explanation queries before entering that path |
| Task1 -> Task2 | Full snapshots and detailed results | One frozen source set, scoped review, full suite pair |
| Task1 self-consistency | Required named tests plus six outcomes and lifecycle | Included above; no provider authority changes |
| Task2 self-consistency | Verification only | No test weakening or source edits by Luna |

Largest risks are wrong rejection attribution, query-side effects, lifecycle leakage and invented reasons. Their behavioral checks are explicit above. Task1 covers each spec section; Task2 covers all final gates. The existing public bridge methods are not modified by this phase. This gated plan was drafted while the final 4C.1 compatibility fix was running because the consumed memory API is unchanged; recheck the recorded gate before dispatch.
