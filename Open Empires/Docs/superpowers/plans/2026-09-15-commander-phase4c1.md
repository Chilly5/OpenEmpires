...
## Task 2: Independent review and phase gate

**Agent:** Sol scoped task reviewer; Luna audit/evidence; Astra integration and security review.

**Consumes:** Task1 report, current source, before/after diff package against Phase4C baseline, detailed test artifacts. **Produces:** review findings and verified 4C.1 gate recorded in progress ledger.

- [ ] Generate a scoped review diff including new files and changes against saved task-start content, not HEAD (inherited untracked work is out of scope).
- [ ] Reviewer verifies spec compliance and code quality without rerunning already-proven unchanged tests. Findings include concrete reproduction or source location and severity. Return fixes to original implementer, with focused covering tests and scoped re-review.
- [ ] Luna checks dependency boundaries and hashes without changing authority/planner/execution code; scans credential shapes without printing matches or reading `.env` values. Records any exceptions for existing game-owned host/context-builder code.
- [ ] Astra verifies user-required tests are present and passing in detailed results, confirms runtime preference-to-preview-to-explicit-approval proof, and resolves all substantive review issues.
- [ ] Mark 4C.1 stable only when all gates pass. Then write the 4C.2 detailed plan using final interfaces; do not implement explanations/context/objective expansion within this task.

## Preflight and planning self-review

| Task/interface pair | Producer and consumer | Check |
| --- | --- | --- |
| Memory -> ConversationState | Copied immutable entries, bounded list | No runtime references or hidden cache |
| ConversationState -> request/bridge | Detached advisory values | Legacy overloads remain valid; no approval semantics |
| UI -> memory | Explicit preference or actual accepted-outcome summaries | Selection/rejection cannot be mislabelled approved |
| Router -> UI/provider | Whole-form conversation vs strategy routing | Conversation never uses tactical fallback |
| Reset -> async publication | UI generation + bridge cancellation/history reset | Late work cannot refill state |
| Task1 -> Task2 | Source snapshots, focused/full evidence, report | Review covers new/untracked files, not only git diff |

Task1 is a single integration deliverable with its test cycle. Task2 verifies rather than implements. Later sub-phases remain required by the overarching design and will receive separate dependency-ordered plans after this gate; this plan does not narrow the overall goal.
